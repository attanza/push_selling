<template src="./outlet_gallery.html"></template>
<script>
...
export default {
  ...
  mounted() {
    ...
    window.eventBus.$on('insert-outlet-media', this.update_media)
  ...
  methods: {
    get_medias(page) {
      var vm = this
      axios.get('/api/gallery/'+this.outlet_id+'/outlet?page=' + page).then((resp) => {
